...
def make_labels_colors_sizes(g, positions: dict, sellers_set, node_ids):
    ...
    num_sellers = positions.get("num_sellers", {})   # может быть {Vertex:...} или {int:...}
    revenues    = positions.get("revenues", {})
    # нормализуем ключи в int
    def k2i(k): return int(k) if not isinstance(k, int) else k
...
def panel_gt_interactive(g, revenue_function: BaseRevenueFunction, pos_prop=None):
    ...
    max_iter_btn = W.BoundedIntText(value=max_iterations, min=1, max=10**6, step=1, description="Max iters")
    base_cost_btn = W.BoundedFloatText(value=base_cost, min=0, max=10**6, step=1, description="Base cost C")
    ...
    def render():
        ...
        with out_stats:
            ...
            box = W.HTML(value=f"<div style='font-family:monospace; font-size:14px;'>{html}</div>")
            display(box)
    ...
    # нужно добавить info
    controls = W.HBox([btn_reset, btn_equil, num_M, max_iter_btn, base_cost_btn, cb_labels])
    # Сделаем правое окно крупным: фиксированная ширина и прокрутка
    stats_box = W.VBox([stats_header, out_stats], layout=W.Layout(width="420px", height="720px", overflow="auto"))
